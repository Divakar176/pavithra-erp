        ...
        <div className="p-8 max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="stripe-card p-6">
                    <div className="text-gray-500 text-[10px] font-bold uppercase tracking-wider mb-2">Total Maintenance Spend</div>
                    <div className="text-3xl font-bold text-white">
                        {formatCurrency(logs.reduce((sum, log) => sum + (log.totalCost || 0), 0))}
                    </div>
                </div>
                <div className="stripe-card p-6">
                    <div className="text-gray-500 text-[10px] font-bold uppercase tracking-wider mb-2">Services This Month</div>
                    <div className="text-3xl font-bold text-white">
                        {logs.filter(l => new Date(l.date).getMonth() === new Date().getMonth()).length}
                    </div>
                </div>
                <div className="stripe-card p-6">
                    <div className="text-gray-500 text-[10px] font-bold uppercase tracking-wider mb-2">Most Frequent Issue</div>
            ...
            <div className="stripe-card p-0 overflow-hidden">
                <div className="p-5 border-b border-[#2A2A2A] bg-[#1A1A1A] flex items-center justify-between">
                    <div className="relative w-64 group">
                        <Filter className="w-4 h-4 absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" />
                        <select 
                            value={filterVehicle}
                            onChange={(e) => setFilterVehicle(e.target.value)}
                            className="w-full pl-11 pr-4 py-2 bg-[#121212] border border-[#2A2A2A] rounded-xl text-sm text-gray-300 focus:outline-none focus:border-[#D8621C] transition-all appearance-none"
                        >
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-[#1A1A1A] text-gray-500 text-[11px] uppercase tracking-wider font-bold">
                                <th className="p-5 border-b border-[#2A2A2A]">Date</th>
                                <th className="p-5 border-b border-[#2A2A2A]">Vehicle</th>
                                <th className="p-5 border-b border-[#2A2A2A]">Service Type</th>
                        ...
                        <tbody className="divide-y divide-[#2A2A2A] bg-[#121212]">
                            {filteredLogs.length === 0 ? (
                                ...
                                <tr key={log.id} className="hover:bg-[#1A1A1A] transition-colors">
                                    ...
                                    <td className="p-5">
                                        <span className="font-bold text-white bg-[#2A2A2A] px-3 py-1 rounded-lg border border-[#333]">
                                            {log.vehicleNumber}
                                        </span>
                                    </td>
                                    <td className="p-5">
                                    ...
                                    <td className="p-5 text-center">
                                        {log.billUrl ? (
                                            <a href={log.billUrl} target="_blank" rel="noreferrer" className="inline-flex p-2 bg-[#2A2A2A] hover:bg-[#333] text-blue-400 rounded-lg transition-colors">
                    ...
                    <div className="stripe-card w-full max-w-lg overflow-hidden border border-[#2A2A2A] shadow-2xl my-8">
                        <div className="flex justify-between items-center p-6 border-b border-[#2A2A2A] bg-[#1A1A1A]">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => setIsModalOpen(false)} className="text-gray-500 hover:text-white bg-[#2A2A2A] hover:bg-[#333] p-1.5 rounded-lg transition-colors">
                                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                </button>
                            </div>
                        </div>
                        <form onSubmit={handleSubmit} className="p-6 space-y-5 bg-[#121212]">
                            {error && (
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select 
                                        ...
                                        className="w-full px-4 py-3 bg-[#1C1C1C] border border-[#2A2A2A] rounded-xl text-white focus:outline-none focus:border-[#D8621C] transition-all"
                                    >
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-400 mb-2">Vendor / Mechanic Details</label>
                                    <input 
                                        type="text" 
                            ...
                            <div className="bg-[#1A1A1A] p-4 rounded-xl border border-[#2A2A2A] space-y-4">
                                {!['Insurance', 'FC Renewal', 'Tax Renewal', 'Permit Renewal', 'Pollution Cert'].includes(formData.serviceType) && (
                                    <>
                                        <h4 className="text-gray-300 font-bold text-sm">Cost Breakdown</h4>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-xs font-bold text-gray-500 mb-1">Spare Parts (₹)</label>
                                                <input 
                                                    type="number" 
                                                    value={formData.sparePartsCost}
                                                    onChange={(e) => setFormData({...formData, sparePartsCost: e.target.value})}
                                                    className="w-full px-3 py-2 bg-[#121212] border border-[#2A2A2A] rounded-lg text-white focus:outline-none focus:border-[#D8621C]"
                                                />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-bold text-gray-500 mb-1">Labour (₹)</label>
                                                <input 
                                                    type="number" 
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full px-4 py-3 bg-[#1C1C1C] border border-[#D8621C]/50 rounded-xl text-white focus:outline-none focus:border-[#D8621C] text-lg font-bold"
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Upload Bill Image (Optional)</label>
                                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-[#2A2A2A] border-dashed rounded-xl hover:border-orange-500/50 transition-colors group relative">
                                    {formData.billUrl ? (
                                        <div className="text-center">
                                            <CheckCircle2 className="mx-auto h-8 w-8 text-emerald-500" />
                                            <p className="mt-2 text-sm text-gray-400">Image attached</p>
                                        ...
                                        <div className="space-y-1 text-center">
                                            <FileText className="mx-auto h-8 w-8 text-gray-500 group-hover:text-orange-400 transition-colors" />
                            ...
                            <div className="pt-4 flex gap-3">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-3 px-4 bg-[#1A1A1A] border border-[#2A2A2A] hover:bg-[#2A2A2A] text-gray-300 font-bold rounded-xl transition-colors">
                                    Cancel
                                </button>
