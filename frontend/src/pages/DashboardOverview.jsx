            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="stripe-card p-0 overflow-hidden">
                        <div className="p-5 border-b border-slate-200 dark:border-[#2A2A2A] flex justify-between items-center">
                            <h2 className="text-slate-900 dark:text-white font-semibold flex items-center">
                                <Navigation className="w-4 h-4 mr-2 text-[#D8621C]" /> Recent Trips
                            </h2>
                            <button className="text-xs font-semibold text-[#D8621C] hover:text-slate-900 dark:text-white transition-colors">View All</button>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                <thead className="text-xs text-slate-500 dark:text-gray-500 bg-slate-100 dark:bg-[#151515] uppercase border-b border-slate-200 dark:border-[#2A2A2A]">
                                    <tr>
                                    ...
                                    {recentTrips.map((trip) => (
                                        <tr key={trip.id} className="border-b border-slate-200 dark:border-[#2A2A2A] hover:bg-white/5 transition-colors">
                                            <td className="px-5 py-4">
                                                <div className="text-slate-900 dark:text-white font-medium">{trip.vehicle?.vehicleNumber}</div>
                                                <div className="text-xs text-slate-500 dark:text-gray-400 font-medium">{trip.driver?.username}</div>
                                                <div className="text-[10px] text-slate-500 dark:text-gray-500 mt-0.5 tracking-wide">
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center text-slate-600 dark:text-gray-300">
                                                    ...
                                                    <ArrowUpRight className="w-3 h-3 mx-2 text-slate-500 dark:text-gray-500" />
                                                    <span>{trip.destination}</span>
                                                </div>
                                            </td>
                                            <td className="px-5 py-4 text-slate-900 dark:text-white font-medium">{formatCurrency(trip.tripCharges)}</td>
                                            <td className="px-5 py-4">
                ...
                <div className="space-y-6">
                    <div className="stripe-card p-5">
                        <h2 className="text-slate-900 dark:text-white font-semibold mb-4 text-sm flex items-center">
                            Vehicle-wise Profit <span className="ml-2 text-xs text-slate-500 dark:text-gray-500 font-normal">(This Month)</span>
                        </h2>
                        <div className="space-y-4">
                            {vehicleProfit.slice(0, 5).map((vp, idx) => (
                                <div key={idx}>
                                    <div className="flex justify-between text-sm mb-1.5">
                                        <span className="text-slate-600 dark:text-gray-300 font-medium">{vp.vehicleNumber}</span>
                                        <span className="text-[#10B981] font-semibold">{formatCurrency(vp.profit)}</span>
                                    </div>
                                    <div className="w-full bg-slate-200 dark:bg-[#2A2A2A] rounded-full h-1.5">
                                        <div
                    ...
                    <div className="stripe-card p-0 overflow-hidden">
                        <div className="p-4 border-b border-slate-200 dark:border-[#2A2A2A] flex items-center justify-between bg-slate-100 dark:bg-[#151515]">
                            <h2 className="text-slate-900 dark:text-white font-semibold text-sm flex items-center">
                                Document Alerts
                            </h2>
                            {alerts.length > 0 && (
                                <span className="bg-red-500 text-slate-900 dark:text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
                                    {alerts.length}
                                </span>
                            )}
                        </div>
                        <div className="p-4 space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar">
                            {alerts.length === 0 ? (
                                <div className="text-slate-500 dark:text-gray-500 text-sm text-center py-4">No urgent alerts</div>
                            ) : (
                                alerts.map((alert, idx) => (
                                    <div key={idx} className={`p-3 rounded-xl border border-white/5 flex items-start ${alert.bg}`}>
                                        ...
                                        <div>
                                            <div className="text-sm font-medium text-slate-900 dark:text-white">{alert.vehicle}</div>
                    ...
                    <div className="stripe-card p-5">
                        ...
                        {lowStockItems.length === 0 ? (
                            <div className="text-slate-500 dark:text-gray-500 text-sm text-center py-4 bg-white/5 rounded-xl border border-white/5">
                            ...
                            <div className="space-y-3">
                                {lowStockItems.map((item) => (
                                    <div key={item.id} className="flex justify-between items-center p-3 bg-white/5 rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                                        <div>
                                            <div className="text-slate-900 dark:text-white text-sm font-medium">{item.itemName}</div>
                                            <div className="text-xs text-slate-500 dark:text-gray-500">Min: {item.reorderLevel}</div>
                                        </div>
